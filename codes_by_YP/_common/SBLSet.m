function options = SBLSet(opts)

%--------------------------------------------------------------------------
% SBLSet creates or alters an options structure for SBL_v3p1.m.
%--------------------------------------------------------------------------
...
% 10^(-8) works, 10^(-5) seems slightly too little
% This depends on Ntheta and should be somehow normalized
options.convergence.error   = 10^(-3); % was 10^(-4), changed by cfm on Feb 23, 2022.
...
options.convergence.maxiter = 1200; %500; %1200;

% solution only accepted at this iteration
options.convergence.min_iteration = 15; 

% status report every xx iterations
options.status_report = 10;
...
% phi parameter for robust version of SBL [0 0.03]
options.phi = 0;

options.flag = 0; % was 1, changed by cfm 2021-08-09
