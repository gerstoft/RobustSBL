...
LL = 1e3, % number of array data vector observations, use 1e3 for debugging, 5e4 for publication quality plots
...
if Number_of_DOAs == 3
    create_search_space;
    fprintf(1,'sparse set of combinations  = %d\n',Mcombinations);
end

m_src = zeros(size(DOA_src));
for k=1:Number_of_DOAs
    m = find(phi_vec == DOA_src(k));
    if ~isempty(m)
        m_src(k) = m;
    else
        error('true source DOAs are not on the grid')
    end
end
...
% keyboard;
    ...
    RMSE_DOA_CBF(isnr)   = sqrt(mean(mean((sort(DOA_CBF(:,1:L:LL))-DOA_src).^2)));                         
    ...
    RMSE_x_CBF(isnr)     = sqrt(mean(MSE_x_CBF)/(LL/L));
    ...
    RMSE_DOA_phase(isnr)   = sqrt(mean(mean((sort(DOA_phase(:,1:L:LL))-DOA_src).^2)));   
    ...
    RMSE_x_phase(isnr)     = sqrt(mean(MSE_x_phase)/(LL/L));
    ...
    RMSE_DOA_mle(isnr)   = sqrt(mean(mean((sort(DOA_mle(:,1:L:LL))-DOA_src).^2))) 
    ...
    RMSE_x_mle(isnr)     = sqrt(mean(mean(MSE_x_mle)/(LL/L))); % this probably doesnt make sense
    ...
    elapsed_time = toc;
    fprintf('elapsed time = %f hrs, completed = %d percent\n---\n',round(elapsed_time/36)/100, round(100 * isnr/length(sigma_vec)));
end
